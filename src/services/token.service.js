...
import { sessions, tokens, users } from '../db/schema/index.js';
...
export async function verifyToken(token, type) {
  ...
  const tokenDoc = await db.query.tokens.findFirst({
    where: and(eq(tokens.userId, payload.sub), eq(tokens.token, token), eq(tokens.type, type)),
  });
  if (!tokenDoc) {
...
export async function generateVerifyEmailToken(userId) {
  const expires = dayjs().add(env.jwt.verifyEmailExpirationMinutes, 'm');
  const verifyEmailToken = generateToken(userId, expires, TOKEN_TYPES.VERIFY_EMAIL);
  await saveToken({
  ...
  return { token: verifyEmailToken, expires };
}
...
export async function findToken(token, type) {
  return db.delete(tokens).where(and(eq(tokens.token, token)), eq(tokens.type, type));
}
...
export async function deleteMany(userId, type) {
  return db.delete(tokens).where(and(eq(tokens.userId, userId), eq(tokens.type, type)));
}
